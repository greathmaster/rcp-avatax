...
if ( ! class_exists( '\SkilledCode\RequestAPI\Base' ) ) :

/**
 * # SkilledCode Plugin Framework API Base Class
 *
...
abstract class Base {
	 ...
	 * @return object request class instance that implements InterfaceRequest
	 */
	 ...
	 * @return object response class instance which implements InterfaceRequest
	 */
	...
	protected function broadcast_request() {
		 ...
		 * }
		 * @param \SkilledCode\RequestAPI\Base $this instance
		 */
	...
	protected function get_request_uri() {
		 ...
		 * @param \SkilledCode\RequestAPI\Base class instance
		 */
	...
	protected function get_request_query() {
		...
		if ( ( $request = $this->get_request() ) && in_array( strtoupper( $this->get_request_method() ), array( 'GET', 'HEAD' ) ) ) {
			...
			$params = is_callable( array( $request, 'get_params' ) ) ? $request->get_params() : array();
	 ...
	 *
	 * Dasherized-Plugin-Name/Plugin-Version (WordPress/WP-Version)
	 *
	 * @since 1.0.0
	 * @return string
	 */
	protected function get_request_user_agent() {
		return sprintf( '%s/%s (WordPress/%s)', str_replace( ' ', '-', $this->get_plugin()->get_plugin_name() ), $this->get_plugin()->get_version(), $GLOBALS['wp_version'] );
	}
	 ...
	 * InterfaceRequest which should be used in the child class API methods
	 * to build the request. The returned InterfaceRequest should be passed
	...
	public function set_tls_1_2_request( $handle, $r, $url ) {
		...
		if ( ! Helpers::str_starts_with( $url, 'https://' ) ) {
			return;
		}
